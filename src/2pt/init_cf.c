/*******************************************************************************
* 2pt/init_cf.c: this file is part of the FCFC program.
...
static int cf_expression(const CONF *conf, CF *cf) {
  ...
  for (int i = 0; i < cf->ncf; i++) {
    ...
    /* There are at most 26 catalogs, so (26^2) different pairs, not including
...
CF *cf_init(const CONF *conf) {
  ...
  if (conf->fcnvt) {
    if (!(cf->coord = cnvt_init())) {
      P_ERR("failed to allocate memory for coordinate interpolation");
  ...
  if (cf->bintype == FCFC_BIN_SPI) {
    ...
    if (!(cf->pbin = malloc(sizeof(real) * (cf->np + 1))) ||
        !(cf->p2bin = malloc(sizeof(real) * (cf->np + 1)))) {
    ...
    for (int i = 0; i <= cf->np; i++) cf->p2bin[i] = cf->pbin[i] * cf->pbin[i];
    cf->p2min = cf->p2bin[0];
  ...
  if (cf->prec != REAL_NAN) {
    ...
    if (cf->bintype == FCFC_BIN_SPI) {
      ...
      int j = 1;
      for (size_t i = 0; i < ntab; i++) {
        if (i + offset < (size_t) (cf->p2bin[j] * cf->prec))
          cf->ptab[i] = j - 1;
        else {
          if (++j > cf->np) {
  ...
  if (!(cf->cnt[0] = calloc(cf->ntot * cf->npc, sizeof(pair_count_t))) ||
  ...
  if (!(cf->pcnt = malloc(sizeof(pair_count_t) * cf->ntot * cf->nthread))) {
