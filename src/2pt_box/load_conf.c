...
Compute the 2-point correlation functions of catalogs in periodic boxes.\n\
  ...
  -b, --box             " FMT_KEY(BOX_SIZE) "        Double\n\
        Side length of the periodic box for distance evaluations\n\
...
SQ_DIST_PREC    = \n\
    ...
    # For periodic boxes, pi bins are not squared, and are truncated directly.\n\
\n\
####################\n\
#  Other settings  #\n\
####################\n\
...
VERBOSE         = \n\
    # Boolean option, indicate whether to show detailed outputs (unset: %c).\n",
      ...
      DEFAULT_ASCII_COMMENT ? "')" : ")", DEFAULT_BINNING, FCFC_BIN_ISO,
...
static cfg_t *conf_read(CONF *conf, const int argc, char *const *argv) {
  ...
  const cfg_param_t params[] = {
    ...
    {'b', "box"         , "BOX_SIZE"       , CFG_DTYPE_DBL , &conf->bsize   },
...
Please re-compile the code with option -DWITH_CFITSIO\n");
  ...
  /* BOX_SIZE */
  CHECK_EXIST_PARAM(BOX_SIZE, cfg, &conf->bsize);
  if (!isfinite(conf->bsize) || conf->bsize <= 0) {
